...
class ST7735:
        ...
        if start_char <= ci <= end_char:
            ...
            for c in char_data:
                ...
                for _ in range(font_height):
                    if c & 0x01:
                        self.fill_rect((x, y), sizes, color)
                    y += sizes[1]
                    c >>= 1
                x += sizes[0]
    ...
    def _reset(self) -> None:
        self.dc(0)
        self.rs.value(1)
        time.sleep_us(500)  # pyright: ignore[reportAttributeAccessIssue]
        self.rs.value(0)
        time.sleep_us(500)  # pyright: ignore[reportAttributeAccessIssue]
        self.rs.value(1)
        time.sleep_us(500)  # pyright: ignore[reportAttributeAccessIssue]
    ...
    def _set_window_loc(self, pos_0: tuple[int, int], pos_1: tuple[int, int]) -> None:
        self._cmd(ST7735.CASET)
        self._window_loc_data[0] = self._offset[0]
        self._window_loc_data[1] = self._offset[0] + int(pos_0[0])
        self._window_loc_data[2] = self._offset[0]
        self._window_loc_data[3] = self._offset[0] + int(pos_1[0])
        self._data(self._window_loc_data)

        self._cmd(ST7735.RASET)
        self._window_loc_data[0] = self._offset[1]
        self._window_loc_data[1] = self._offset[1] + int(pos_0[1])
        self._window_loc_data[2] = self._offset[1]
        self._window_loc_data[3] = self._offset[1] + int(pos_1[1])
        self._data(self._window_loc_data)
    ...
    def init(self) -> None:
        ...
        time.sleep_us(150)  # pyright: ignore[reportAttributeAccessIssue]
        self._cmd(ST7735.SLPOUT)
        time.sleep_us(500)  # pyright: ignore[reportAttributeAccessIssue]

        data = bytearray([0x01, 0x2C, 0x2D])
        ...
        data = bytearray([0x01, 0x2C, 0x2D, 0x01, 0x2C, 0x2D])
        self._cmd(ST7735.FRMCTR3)
        self._data(data)
        time.sleep_us(10)  # pyright: ignore[reportAttributeAccessIssue]
        ...
        data = bytearray([0xA2, 0x02, 0x84])
        self._cmd(ST7735.PWCTR1)
        self._data(data)
        ...
        data = bytearray([0x8A, 0x2A])
        self._cmd(ST7735.PWCTR4)
        self._data(data)

        data = bytearray([0x8A, 0xEE])
        ...
        self._cmd(ST7735.GMCTRP1)
        self._data(dataGMCTRP)
        ...
        self._cmd(ST7735.GMCTRN1)
        self._data(dataGMCTRN)

        time.sleep_us(10)  # pyright: ignore[reportAttributeAccessIssue]

        self._cmd(ST7735.DISPON)
        time.sleep_us(100)  # pyright: ignore[reportAttributeAccessIssue]
